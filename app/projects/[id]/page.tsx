...
export default async function ProjectDetail({ params }: Props) {
  ...
  const canView = role === "ADMIN" || role === "AGENT" || role === "SALES";
  ...
  const displayStatus = project.type === "WEBSITE"
    ? deriveProjectStatus({
        ...
        webDokuConfirmedAt: website?.webDocumentation?.confirmedAt,
      })
    : project.status;
  ...
  return (
    <div className="p-2 md:p-6 space-y-4 md:space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
                      ...
                      <>
                        ...
                        {website.webDocumentation.confirmedAt && (
                          <p className="text-xs text-blue-600 dark:text-blue-400 mt-1 flex items-center gap-1">
                            ...
                            Vom Kunden bestätigt am {new Date(website.webDocumentation.confirmedAt).toLocaleDateString("de-DE")} um {new Date(website.webDocumentation.confirmedAt).toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" })} Uhr von {website.webDocumentation.confirmedByName}{website.webDocumentation.confirmedByIp && ` (IP: ${website.webDocumentation.confirmedByIp})`}
                          </p>
                        )}
                        {/* Kunden-Ablehnung */}
        ...
        {canView && website?.webDocumentation && (() => {
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                ...
                {(() => {
                  ...
                  if (canEdit && allMaterialComplete && materialStatusNotComplete) {
                    return (
        ...
        {/* Texterstellung Card - Only show when Textit is active and all bullet points are marked suitable */}
        {canView && website && website.textit && website.textit !== "NEIN" && (() => {
          const webDoku = website.webDocumentation;
          if (!webDoku) return null;

          // Check if all texts that need to be submitted are marked as suitable
          const menuItemsNeedingTexts = webDoku.menuItems?.filter((m) => m.needsTexts) || [];
          const allMenuTextsReviewed = menuItemsNeedingTexts.every(
            (m) => m.textSubmission?.submittedAt && m.textSubmission?.suitable === true
          );

          // Check general text submission if needed
          const generalTextOk = !webDoku.materialNotesNeedsTexts ||
            (webDoku.generalTextSubmission?.submittedAt && webDoku.generalTextSubmission?.suitable === true);

          const allTextsApproved = menuItemsNeedingTexts.length > 0 && allMenuTextsReviewed && generalTextOk;

          if (!allTextsApproved) return null;

          const texterstellung = website.texterstellung;

          // Calculate texterstellung statistics if it exists
          const texterstellungStats = texterstellung ? {
            total: texterstellung.items.length,
            pending: texterstellung.items.filter(i => i.status === "PENDING").length,
            draft: texterstellung.items.filter(i => i.status === "DRAFT").length,
            submitted: texterstellung.items.filter(i => i.status === "SUBMITTED").length,
            revisionRequested: texterstellung.items.filter(i => i.status === "REVISION_REQUESTED").length,
            approved: texterstellung.items.filter(i => i.status === "APPROVED").length,
          } : null;

          const isCompleted = texterstellungStats && texterstellungStats.approved === texterstellungStats.total;
          const hasItemsToReview = texterstellungStats && texterstellungStats.submitted > 0;
          const hasRevisions = texterstellungStats && texterstellungStats.revisionRequested > 0;
                ...
                {texterstellung ? (
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-gray-900 dark:text-white">
                          {isCompleted
                            ? "Abgeschlossen"
                            : hasRevisions
                            ? `${texterstellungStats!.revisionRequested} Korrektur${texterstellungStats!.revisionRequested > 1 ? "en" : ""} angefordert`
                            : hasItemsToReview
                            ? `${texterstellungStats!.submitted} beim Kunden zur Prüfung`
                            : "In Bearbeitung"
                          }
                        </p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          {texterstellungStats!.approved} von {texterstellungStats!.total} Texte fertig
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 flex-shrink-0">
                      {role === "ADMIN" && (
                        <DeleteTexterstellungButton
                          texterstellungId={texterstellung.id}
                          projectId={project.id}
                        />
                      )}
                      <Link
                        href={`/projects/${project.id}/texterstellung`}
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900 dark:text-white">Bereit zum Starten</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          Alle Stichpunkte wurden als geeignet markiert
                        </p>
                      </div>
                    </div>
                    <Link
                      href={`/projects/${project.id}/texterstellung`}
                      className="inline-flex items-center gap-2 px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors font-medium text-sm shadow-sm"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                      </svg>
