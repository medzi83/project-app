...
export async function GET(request: NextRequest) {
  ...
  const size = (sizeParam ? parseInt(sizeParam) : 96) as 48 | 96 | 192 | 256;
  ...
  try {
    const thumbnailLink = await getThumbnailLink(agency, libraryId, filePath, size);
    ...
    console.error('LuckyCloud thumbnail error:', error);
    return NextResponse.json({
      success: false,
