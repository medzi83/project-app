...
export class FroxlorClient {
  ...
  async findCustomerByNumber(customerNumber: string, debug = false): Promise<FroxlorCustomer | null> {
    try {
      ...
      const result = await this.request<any>('Customers.listing');

      if (result.status === 200 && result.data) {
        // Froxlor 0.10.38+ returns {count: X, list: [...]}
        let customers: any[] = [];
        ...
        if (result.data.list && Array.isArray(result.data.list)) {
          ...
          customers = Object.values(result.data);
        }
  ...
  async getPhpConfigs(): Promise<FroxlorPhpConfig[]> {
    try {
      const result = await this.request<any>('PhpSettings.listing');
      ...
      if (result.status === 200 && result.data) {
        ...
        if (result.data.list && Array.isArray(result.data.list)) {
          ...
          configs = Object.values(result.data);
        }

        return configs.filter((c) => c && typeof c === 'object');
      }
  ...
  async getCustomerDomains(customerId: number): Promise<FroxlorDomain[]> {
    try {
      const result = await this.request<any>('Domains.listing', {
      ...
      if (result.status === 200 && result.data) {
        ...
        if (result.data.list && Array.isArray(result.data.list)) {
          ...
          domains = Object.values(result.data);
        }

        // Filter by customerid (API might return all domains despite parameter)
        return domains.filter((d) => d && typeof d === 'object' && d.customerid == customerId);
      }
